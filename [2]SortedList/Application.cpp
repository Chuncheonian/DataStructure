...
int Application::GetCommand() {
	...
	cout << "\t   4 : Search content by ID(SS)" << endl;
	cout << "\t   5 : Search content by ID(BS)" << endl;
	cout << "\t   6 : Search content by Address" << endl;
...
// Get a address from keyboard and Display a record with input address. 
int Application::SearchByAddress() {		// case 6
	ItemType item;
	item.SetAddressFromKB();

	if (SearchAllContentByAddress(item))
		return 1;
	return 0;
}

// get a charaaddresscter from keyboard and display a record with input address.
int Application::SearchAllContentByAddress(ItemType& data) {
	ItemType tmp;
	int result = 0;

	m_List.ResetList();
	while (m_List.GetNextItem(tmp) != -1) {
		if (tmp.GetAddress() == data.GetAddress()) {
			if (result == 0)
