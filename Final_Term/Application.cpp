...
int Application::DeleteContents() {
  ...
  FavoriteContent musicContent;
  FavoriteContent photoContent;
  FavoriteContent videoContent;
  musicContent.SetTime(mainContent.GetTime());
  photoContent.SetTime(mainContent.GetTime());
  videoContent.SetTime(mainContent.GetTime());

  if (m_favoriteMusicList.Retrieve_BinS(musicContent)) // favoritemusiclist에 있으면 삭제
    m_favoriteMusicList.Delete(musicContent);
...
int Application::ReplaceContents() {
  ...
  if (found == true) {
    ...
    // favoritemusiclist에서 변경 작업
    if (m_favoriteMusicList.Retrieve_BinS(musicContent)) {
      if (mainContent.GetType() == 2) { // music -> photo 경우
        m_favoriteMusicList.Delete(musicContent);
        photoContent.SetRecordMC(mainContent);
        photoContent.SetCondition(musicContent.GetCondition());
        photoContent.SetViewNumber(musicContent.GetViewNumber());
        m_favoritePhotoList.Add(photoContent);
      }
      else if (mainContent.GetType() == 3) { // music -> video인 경우
        m_favoriteMusicList.Delete(musicContent);
        videoContent.SetRecordMC(mainContent);
        videoContent.SetCondition(musicContent.GetCondition());
        videoContent.SetViewNumber(musicContent.GetViewNumber());
        m_favoriteVideoList.Add(videoContent);
      }
      else { // 타입이 변경되지 않은 경우
        musicContent.SetRecordMC(mainContent);
        m_favoriteMusicList.Replace(musicContent);
      }
    }

    // favoritephotolist에서 변경 작업
    if (m_favoritePhotoList.Retrieve_BinS(photoContent)) {
      if (mainContent.GetType() == 1) { // photo -> music 경우
        m_favoritePhotoList.Delete(photoContent);
        musicContent.SetRecordMC(mainContent);
        musicContent.SetCondition(photoContent.GetCondition());
        musicContent.SetViewNumber(photoContent.GetViewNumber());
        m_favoriteMusicList.Add(musicContent);
      }
      else if (mainContent.GetType() == 3) { // photo -> video인 경우
        m_favoritePhotoList.Delete(photoContent);
        videoContent.SetRecordMC(mainContent);
    ...
    if (m_favoriteVideoList.Retrieve_BinS(videoContent)) {
      if (mainContent.GetType() == 1) { // video -> music 경우
        m_favoriteVideoList.Delete(videoContent);
        musicContent.SetRecordMC(mainContent);
        musicContent.SetCondition(videoContent.GetCondition());
        musicContent.SetViewNumber(videoContent.GetViewNumber());
        m_favoriteMusicList.Add(musicContent);
      }
      if (mainContent.GetType() == 2) { // video -> photo인 경우
        m_favoriteVideoList.Delete(videoContent);
        photoContent.SetRecordMC(mainContent);
...
int Application::AddFavoriteContent() {
  ...
  if (mainContent.GetType() == 1) {   // Type == Music
    FavoriteContent musicContent(mainContent); // musicContent 변수 세팅

    if (m_favoriteMusicList.GetLength() >= 1) { // FavoriteMusicList에 이미 컨텐츠가 존재하면 m_condition 일치하게 만들기
      FavoriteContent temp;
      m_favoriteMusicList.ResetList();
      m_favoriteMusicList.GetNextItem(temp);
      musicContent.SetCondition(temp.GetCondition());
    }

    int status = m_favoriteMusicList.Add(musicContent);
    ...
    FavoriteContent photoContent(mainContent); // photoContent 변수 세팅
  ...
  else if (mainContent.GetType() == 3) {  // Type == Video
    FavoriteContent videoContent(mainContent); // videoContent 변수 세팅
...
int Application::DeleteFavoriteContent() {
  ...
  if (mainContent.GetType() == 1) { // Type == Music
    ...
    int status = m_favoriteMusicList.Delete(musicContent);
...
void Application::DisplayAllFavoriteContent() {
  cout << "\n\t--Current Favorite Music list--" << endl;

  m_favoriteMusicList.ResetList();
  int length = m_favoriteMusicList.GetLength();

  for (int i = 0; i < length; i++) {
    FavoriteContent musicContent;
    m_favoriteMusicList.GetNextItem(musicContent);
    musicContent.SetViewNumber(musicContent.GetViewNumber() + 1); // 조회 수 1 증가
    m_favoriteMusicList.Replace(musicContent);                    // 조회 수 1 증가 된 musicContent favoritePhotoList에 업데이트
    cout << musicContent << endl;                                 // 출력
  }

  cout << "\n\t--Current Favorite Photo list--" << endl;

  m_favoritePhotoList.ResetList();
  length = m_favoritePhotoList.GetLength();
...
void Application::DisplayDetailFC() {
  ...
  if (mainContent.GetType() == 1) { // Type == Music
    ...
    if (m_favoriteMusicList.Retrieve_BinS(musicContent)) {
      musicContent.SetViewNumber(musicContent.GetViewNumber() + 1); // 조회 수 1 증가
      m_favoriteMusicList.Replace(musicContent);                    // 조회 수 1 증가 된 musicContent favoriteMusicList에 업데이트
      mainContent.SetTime(musicContent.GetTime());
      m_masterList.RetrieveItem(mainContent, found);
      cout << "\n\tCurrent list" << endl;
      cout << mainContent; // 출력
    }
    else
      cout << "\n\tNot exist in Favorite Music list" << endl;
  }
  ...
  else if (mainContent.GetType() == 3) { // Type == Photo
    FavoriteContent videoContent;
    videoContent.SetTime(mainContent.GetTime());
...
void Application::RearrangeFC() {
  int listType;
  int condition;
  cout << "\n\tChoose the list Rearranging (1. Favorite Music List, 2. Favorite Photo List, 3. Favorite Video List) : ";
  cin >> listType;

  if (listType == 1) {  // Type -> Music
    SortedList<FavoriteContent> tmpPhotoList = m_favoriteMusicList; // Favorite Music Content List.
    int length = m_favoriteMusicList.GetLength();
    ...
    for (int i = 0; i < length; i++) {
      FavoriteContent musicContent;
      tmpPhotoList.GetNextItem(musicContent);
      musicContent.SetCondition(condition);
      m_favoriteMusicList.Add(musicContent);
    }
    DisplayAllFavoriteContent();
  ...
  else if (listType == 2) {  // Type -> Photo
    SortedList<FavoriteContent> tmpPhotoList = m_favoritePhotoList; // Favorite Photo Content List.
  ...
  else if (listType == 3) { // Type -> Video
    SortedList<FavoriteContent> tmpVideoList = m_favoriteVideoList; // Favorite Video Content List.
