...
/**
 * Iterator class for SortedDoublyLinkedList.  (not using header and trailer nodes).
 * 제시된 ADT에 대한 견해
...
private:
	const SortedDoublyLinkedList<T>& m_List;	///< 사용할 리스트의 참조 변수
	DoublyNodeType<T>* m_pCurPointer;	///< Node pointer for pointing current node to use iteration.

public:
	/**
     * Initialize m_List and m_pCurPointer. (constructor overloading), Copy Constructor.
     */
	DoublyIterator(const SortedDoublyLinkedList<T>& list) : m_List(list), m_pCurPointer(list.m_pFirst) {}
