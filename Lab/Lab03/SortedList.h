...
// Make list empty.
template <class T>
void SortedList<T>::MakeEmpty() const {
	m_length = 0;
}
...
int SortedList<T>::Add(T _item) {
	...
	if (m_length == 0) {
		...
		for (int i = 0; i < m_length; i++)  {
			if (m_pList[i] < _item) {   // new PK is larger that existed PK in list
			...
			else if (m_pList[i] > _item) {  // new PK is smaller that existed PK in list
...
int SortedList<T>::Retrieve_BinS(T& _item) {
	...
	while (first <= last && !found) {
		...
		if (_item < m_pList[mid])
			last = mid - 1;

		else if (_item > m_pList[mid])
			first = mid + 1;
