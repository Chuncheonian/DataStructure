...
// Favorite Photo Content information class
class FavoritePhotoContent {
protected:
	...
	int		m_condition;		// Member Variable for Rearranging all record of FavoritPhotoList. (1. createTime, 2. contentName, 3. viewNumber)
	int		m_viewNumber;		// view number.

public:
	// default constructor.
	FavoritePhotoContent() {
		m_contentName = "";
		m_createTime = "";
		m_character = "";
		m_location = "";
	...
	FavoritePhotoContent(MultimediaContent& _mc) {
		SetName(_mc.GetName());
		SetTime(_mc.GetTime());
	 ...
	 * @param	_contentName	content name.
	 */
	 ...
	 * @pre		right FavoritePhotoContent class should be initialized.
	 * @post	content record in left FavoritePhotoContent class is set.
	 * @param	_data	target FavoritePhotoContent for assigning.
	 */
	void operator=(const FavoritePhotoContent& _data);

	/**
	 * @brief	Compare corresponding condition in two FavoritePhotoContent class
	 * @pre		Two FavoritePhotoContent class should be initialized.
	 * @post	None.
	 * @param	_data	target FavoritePhotoContent for comparing.	
	 * @return	Return true If Two member variables corresponding Condition are the same, Otherwise return false.
	 */    
	bool operator==(const FavoritePhotoContent& _data);

	/**
	 * @brief	Compare two FavoritePhotoContent by member variable corresponding Condition.
	 * @pre	    Two FavoritePhotoContent should be initialized.
	 ...
	 */
	bool operator<(FavoritePhotoContent& _data);
	...
	/**
	 * @brief	Print record.
	 * @pre		FavoritePhotoContent class should be initialized.
	 * @post	content record is on screen.
	 * @param	cout	ostream instance
	 * @param	_data	target FavoritePhotoContent for printing.
	 */
	friend ostream& operator<<(ostream& cout, const FavoritePhotoContent& _data) {
