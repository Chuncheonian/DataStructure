...
public:
	...
	*/    
	 ...
	 * @post	none.
	 * @param	_item	target ItemType for comparing.	
	 * @return	return true if this->m_contentName == _item->m_contentName,
	 * 			otherwise return false.
	 */    
    bool operator==(const ItemType _item) const {
        if (m_contentName == _item.m_contentName)
            return true;
        else
            return false;
    }
	 ...
	 * @pre		right ItemTypeclass should be initialized.
	 * @post	content record in left ItemTypeclass is set.
	 * @param	_item	target ItemTypefor assigning.
	 */
    void operator=(const ItemType _item) {
        m_contentName 		= _item.m_contentName;
        m_contentType 		= _item.m_contentType;
        m_createTime 		= _item.m_createTime;
        m_character 		= _item.m_character;
        m_contentCategory	= _item.m_contentCategory;
        m_eventName			= _item.m_eventName;
		m_eventDescription	= _item.m_eventDescription;
        m_contents			= _item.m_contents;
	}
	 ...
	 */
	bool operator<(const ItemType& _item) const {
		if (this->m_contentName < _item.GetName())
			return true;
		return false;
	}
