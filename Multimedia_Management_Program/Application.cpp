...
int Application::AddContents() {
	...
	int status = m_masterList.Add(mainContent);

	if (status == -1) {     // masterList 안이 꽉 찰 경우
		cout << "\n\tList is full" << endl;
		return -1;
	}

	else if (status == 0) {     // masterList에 이미 존재할 경우
...
int Application::DeleteContents() {
	...
	m_masterList.Get(mainContent);

	int status = m_masterList.Delete(mainContent);

	if (status == -1) { // masterList 안이 비어있을 경우
		cout << "\tList is empty" << endl;
		return -1;
	}

	else if (status == 0) {     // masterList에 없는 경우
		cout << "\tThe content doesn't exist" << endl;
		return 0;
	}

	else {		// masterlist에서 제거
...
int Application::ReplaceContents() {
	...
	// masterlist에 존재하는 경우
	if (m_masterList.Get(mainContent)) {
		oldMC = mainContent;
		...
		m_masterList.Replace(mainContent);	// masterlist에 교체 완료
